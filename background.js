...
function buildSystemPrompt(s, hasImages, hasAnnotations) {
  ...
  lines.push(
    ...
    "- Under 280 characters total, including any @handle or punctuation — X's hard post limit. This is a strict ceiling, not a target: count the characters before answering, and cut a clause rather than go over.",
...
function buildDigestSystemPrompt(s, preFiltered) {
  return [
    ...
    "- draft.text must be under 280 characters total — X's hard post limit. This is a strict ceiling, not a target: count the characters before answering, and cut a clause rather than go over.",
    "- " + buildFactInventionRule(),
